export default async function handler(req, res) {
  ...
  `;
    ...
    try {
      ...
      const replyText = gptData.choices?.[0]?.message?.content || "Sorry, I didn’t understand that.";
  
      // Get TTS audio for reply
      const ttsRes = await fetch("https://api.openai.com/v1/audio/speech", {
        ...
        body: JSON.stringify({
          model: "tts-1",
          voice: "shimmer", // Change to "nova", "fable", etc if preferred
          input: replyText
        })
      });
